...
def quiz_list(request):
    ...
    categories = Category.objects.filter(is_active=True).annotate(
        quiz_count=Count('quiz', filter=Q(quiz__ai_generated=True))
    ).order_by('name')
...
def categories(request):
    ...
    categories = Category.objects.filter(is_active=True).annotate(
        quiz_count=Count('quiz', filter=Q(quiz__is_active=True, quiz__is_public=True, quiz__ai_generated=True))
