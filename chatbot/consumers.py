...
class ChatbotConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        cache_key = f"chatbot_active_{self.user.id}"
    ...
    async def handle_chat_message(self, data):
        # Use session_id from URL route instead of message data
        ...
        rate_limit_key = f"chatbot_rate_limit_{self.user.id}"
        ...
        if response_data:
            # Cache the latest response for quick access
            response_cache_key = f"chatbot_last_response_{session_id}"
            cache.set(response_cache_key, response_data, timeout=300)  # 5 minutes
    ...
    async def handle_typing(self, data):
        # Use Redis to track typing status across multiple server instances
        typing_key = f"chatbot_typing_{self.session_id}_{self.user.id}"
        is_typing = data.get('is_typing', False)
        
        if is_typing:
            cache.set(typing_key, True, timeout=10)
        else:
            cache.delete(typing_key)
    ...
    def process_chat_message(self, session_id, user_message, provider='perplexity'):
        try:
            ...
            session_cache_key = f"chatbot_session_{session_id}"
            session = cache.get(session_cache_key)
            
            if not session:
                session = ChatSession.objects.get(id=session_id, user=self.user)
                cache.set(session_cache_key, session, timeout=300)  # 5 minutes
