...
class AIChatService:
    ...
    def get_chat_response(self, user_message, session, user, provider=None):
        ...
        if cached_response:
            ...
            return cached_response['response'], cached_response['data']
        ...
        # Get user's chatbot settings with caching
        from ..models import ChatbotSettings
        
        settings_cache_key = f"chatbot_settings_{user.id}"
        settings_obj = cache.get(settings_cache_key)
        
        if not settings_obj:
            try:
                settings_obj = ChatbotSettings.objects.get(user=user)
                cache.set(settings_cache_key, settings_obj, timeout=600)
    ...
    def _build_conversation_context(self, session, user, response_style):
        """Build conversation context from session history with caching"""
        from ..models import BotMessage

        # Try to get context from cache
        context_cache_key = f"conversation_context_{session.id}_{response_style}"
...
Previous conversation:"""
        ...
        cache.set(context_cache_key, context, timeout=300)
        return context
    ...
    def _get_fallback_response(self, user_message):
        ...
        # Simple keyword-based responses with caching
        fallback_cache_key = f"fallback_response_{hashlib.md5(user_message.encode()).hexdigest()}"
        ...
        cache.set(fallback_cache_key, response, timeout=300)
        return response
